...
import org.rust.RsBackgroundTaskQueue
...
class CratesLocalIndexServiceImpl : CratesLocalIndexService, Disposable {
    ...
    private fun loadAsync() {
        ApplicationManager.getApplication().executeOnPooledThread {
            ...
            MACRO_LOG.debug("Loading CratesLocalIndexService finished")
        }
    }

    override fun getCrate(crateName: String): RsResult<CargoRegistryCrate?, Error> =
        query { it.getCrate(crateName) }

    override fun getAllCrateNames(): RsResult<List<String>, Error> =
        query(CratesLocalIndexServiceImplInner::getAllCrateNames)

    private fun <T> query(action: (CratesLocalIndexServiceImplInner) -> T): RsResult<T, Error> {
        if (isUpdating) {
...
private class CratesLocalIndexServiceImplInner(
    ...
    companion object {
        fun tryCreate(
            ...
            if (indexedCommitHash == INVALID_COMMIT_HASH) {
...
private class CratesLocalIndexUpdateTask(
    private val cargoRegistryIndexPath: Path,
    private val indexedCommitHash: String,
    private val writeCratesUpdate: (CratesUpdate) -> Unit,
    private val onErrorCb: (Error.InternalError) -> Unit,
    ...
    override fun run(indicator: ProgressIndicator) {
        ...
        val update = try {
            ...
            LOG.warn(e)
            onErrorCb(Error.InternalError.NoCargoIndex(cargoRegistryIndexPath))
            return
        } catch (e: IOException) {
            onErrorCb(Error.InternalError.RepoReadError(cargoRegistryIndexPath, e.toString()))
            LOG.warn(e)
            return
        }
        ...
        if (update.updatedCrates.isNotEmpty()) {
            ProgressManager.getInstance().executeNonCancelableSection {
                try {
                    ...
                    LOG.warn(e)
                    onErrorCb(Error.InternalError.PersistentHashMapWriteError(e.toString()))
                }
            }
        }
    }
                ...
                .map { (objectId, name) ->
                    ...
                    fileReader.forEachLine { line ->
                        ...
                        try {
                            ...
                            // TODO treeWalk.pathString?
                            LOG.warn("Failed to parse JSON from $ {treeWalk.pathString}, line $line", e)
                        }
                    }
...
val LOG: Logger = logger<CratesLocalIndexServiceImpl>()
...
private const val CRATES_INDEX_VERSION: Int = 6
